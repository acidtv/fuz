...
2. **Matcher** is a three-tier scorer (`score_alignment` in `src/matcher.rs`). **Literal substring matches** sit in a high band (`LITERAL_BASE = 1024` + boundary bonus) — any literal strictly beats any subsequence-only match. **Within-word subsequences** (all gap bytes between matched positions are alphanumeric) score `SINGLE_WORD_BASE + BOUNDARY_BONUS * boundary_hits - SPREAD_PENALTY * excess`; the floor keeps them clearly above cross-word matches. **Cross-word subsequences** (some gap contains a non-alphanumeric byte) score `BOUNDARY_BONUS * boundary_hits - SPREAD_PENALTY * excess` and can go negative, ranking junk like `requires` from `current … acquires` below real matches. A boundary is line start, any position preceded by a non-alphanumeric byte, *or* a CamelCase transition (lowercase→uppercase). Nothing is filtered out by score — TopK alone bounds visible results. The constants are interdependent; don't tweak `BOUNDARY_BONUS` / `SPREAD_PENALTY` / `LITERAL_BASE` / `SINGLE_WORD_BASE` without rerunning the score-ordering tests (`subseq_within_one_word_scores_positive`, `subseq_spread_across_words_scores_nonpositive`, `camelcase_transition_counts_as_boundary`, `within_word_no_boundary_scores_below_within_word_with_boundary`).
